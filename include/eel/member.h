...
template<typename T>
struct member {
    static_assert(!std::is_array_v<T>, "eel::member doesn't support arrays");
...
struct indirect_member {
    static_assert(
            std::is_pointer_v<T> || std::is_reference_v<T> || eel::is_smart_pointer_v<T>,
            "eel::indirect_member only support references and pointers");
    ...
    constexpr indirect_member(V&& val)
        : value_{detail::memberInit<T>(std::forward<V>(val))}
    {
        precondition(value_ != nullptr, EEL_LINE);
    }
    ...
    template<
            typename CheckType = T,
            std::enable_if_t<eel::is_smart_pointer_v<CheckType> || std::is_pointer_v<CheckType>>* = nullptr>
    constexpr auto operator->()
    {
        if constexpr (eel::is_smart_pointer_v<T>)
            return value_.get();
        else
            return value_;
    }
